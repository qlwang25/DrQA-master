...
def store_contents(data_path, save_path, preprocess, num_workers=None):
    ...
    with tqdm(total=len(files)) as pbar:
        for pairs in tqdm(workers.imap_unordered(get_contents, files[:2])):
            count += len(pairs)
...
if __name__ == '__main__':
    ...
    data_path = os.path.join(abs_path, 'data/datasets/sourcedata')
    save_path = os.path.join(abs_path, 'data/wikipedia/db.db')

    parser = argparse.ArgumentParser()
    parser.add_argument('--data_path', type=str, help='/data/datasets/sourcedata', default=data_path)
    parser.add_argument('--save_path', type=str, help='/data/wikipedia/db.db', default=save_path)
