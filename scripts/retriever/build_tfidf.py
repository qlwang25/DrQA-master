...
def get_count_matrix(args, db, db_opts):
    ...
    for i, batch in enumerate(batches):
        logger.info('-' * 24 + 'Batch %d/%d' % (i + 1, len(batches)) + '-' * 24)
        k = 0
...
if __name__ == '__main__':
    ...
    db_path = os.path.join(abs_path, 'data/wikipedia/wikipedia.db')
    out_path = os.path.join(abs_path, 'data/wikipedia/')
    ...
    basename += ('-tfidf-ngram=%d-hash=%d-tokenizer=%s-run' %
